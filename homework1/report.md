...
下方的記憶體使用量表格呈現了各演算法在不同資料量下的測量結果（單位為 KB）。然而，表中部分數據不完全符合各演算法理論上的空間複雜度（例如，O(1) 的演算法顯示非零或波動的記憶體增量；O(n) 的演算法增量趨勢與 n 不完全線性相關）。這可能與我們在申論及開發報告中提到的 Vector 內部記憶體管理策略（如預留空間）以及程式記憶體使用量 (WorkingSetSize) 的測量方式有關，這些因素導致了測量結果的波動和偏差，使其無法精確反映演算法本身的空間複雜度理論值。
